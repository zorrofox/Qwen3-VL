...
def main():
    ...
    # 5.5 FP8 量化（可选）：用 Qwix QT wrapper，把 Dense matmul 在 forward 时 cast 到 FP8。
    # 上次失败（commit f30026f）的根因：
    #   1) module_path=r'.*Dense.*' 匹配 Flax 类名，但 Qwix 实际匹配 instance name
    #      （如 'q_proj'/'gate_proj'，或未命名时 'Dense_0'）→ 0 layer 命中
    #   2) weight_qtype='fp8' 字符串无效；Qwix 0.1.6+ 要求 jnp.float8_e4m3fn dtype 对象
    ...
    # 6.1 PRINT_MODULE_PATHS=1：dry-run 打印所有 Dense kernel 的 PyTree path 后退出。
    # 这是写正确 Qwix module_path 正则的真理之源——上次失败就是因为靠猜。
    if os.environ.get("PRINT_MODULE_PATHS", "").lower() in ("1", "true"):
        logger.info("=== PRINT_MODULE_PATHS=1: dumping all kernel paths ===")
        n = 0
        for path, leaf in jax.tree_util.tree_leaves_with_path(params):
            path_str = '/'.join(str(p.key) if hasattr(p, 'key') else str(p) for p in path)
            if 'kernel' in path_str:
                logger.info(f"PARAM_PATH: {path_str} shape={tuple(leaf.shape)} dtype={leaf.dtype}")
                n += 1
        logger.info(f"=== Total {n} kernel paths printed; exiting ===")
        sys.exit(0)
    ...
    # 8.1 FP8 验证：Qwix QT 模式下 weights 保持 BF16（forward 时动态 cast 到 FP8），
    # 所以不能靠 params dtype 检测。改用 jaxpr 扫描，看 forward 计算图中是否出现 float8 op。
    # 这是真正的 ground truth：编译图里有 fp8 = FP8 已生效。
    if training_args.enable_fp8:
        try:
            dummy_input_ids = jnp.ones((1, 4), dtype=jnp.int32)
            dummy_pos_ids = jnp.zeros((3, 1, 4), dtype=jnp.int32)
            jaxpr = jax.make_jaxpr(model.apply)({"params": params}, dummy_input_ids, dummy_pos_ids)
            jaxpr_str = str(jaxpr)
            fp8_op_count = jaxpr_str.count('float8_e4m3fn') + jaxpr_str.count('float8')
            if fp8_op_count == 0:
                logger.error(
                    "FP8 jaxpr scan: 0 float8 ops in forward graph. "
                    "Qwix module_path regex likely did not match any layer. "
                    "Run PRINT_MODULE_PATHS=1 and check train.py:345-368."
                )
                raise RuntimeError(f"FP8 not applied (0 float8 ops in forward jaxpr)")
            logger.info(f"FP8 verified: {fp8_op_count} float8 ops in forward jaxpr (size={len(jaxpr_str)} chars)")
        except Exception as e:
            if isinstance(e, RuntimeError):
                raise
            logger.warning(f"FP8 jaxpr scan failed (non-fatal): {type(e).__name__}: {e}; relying on step_time as fallback")
