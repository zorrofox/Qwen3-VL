...
def _read_video_decord(
    ...
    Returns:
        np.ndarray: the video array with shape (T, C, H, W).
    """
    import decord
    ...
    idx = np.linspace(start_frame, end_frame, nframes).round().astype(np.int64).tolist()
...
def _read_video_torchcodec(
    ...
    if hasattr(video, 'numpy'):
        video = video.numpy()  # torch tensor -> numpy
...
def get_video_reader_backend() -> str:
    if FORCE_QWENVL_VIDEO_READER is not None:
        ...
        raise RuntimeError(
            "No video reader backend available. Please install either 'decord' or 'torchcodec'."
        )
    print(f"qwen-vl-utils using {video_reader_backend} to read video.", file=sys.stderr)
    return video_reader_backend


def _resize_video_frames(video_np: np.ndarray, height: int, width: int) -> np.ndarray:
    """Resize video frames using PIL. Input/output: (T, C, H, W)."""
    resized = []
    for t in range(video_np.shape[0]):
        frame = video_np[t].transpose(1, 2, 0)  # CHW -> HWC
        if frame.dtype != np.uint8:
            frame = np.clip(frame, 0, 255).astype(np.uint8)
        pil_frame = Image.fromarray(frame)
        pil_frame = pil_frame.resize((width, height), Image.BICUBIC)
        resized.append(np.array(pil_frame).transpose(2, 0, 1))  # HWC -> CHW
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
                return_video_metadata: bool = False) -> Union[np.ndarray, List[Image.Image]]:
    ...
    video = _resize_video_frames(video, resized_height, resized_width)
...
) -> Tuple[Optional[List[Image.Image]], Optional[List[Union[np.ndarray, List[Image.Image]]]], Optional[Dict[str, Any]]]:
